...
void OriginalLinearOperation::multiply_mm_transposed_b(const double* matrixA, int rowsA, int colsA,
    ...
    for (int i = 0; i < rowsA; ++i) {
        ...
        double* crow = result + i * colsB;         // i th row start pointer of result
        for (int j = 0; j < colsB; ++j) {
            double sum = 0.0;
...
// TODO
// 目前实现了并行
void OriginalLinearOperation::multiply_mv_row_major_opt(const double* matrix, int rows, int cols,
                                                  const double* vector, double* result) {
    #pragma omp parallel for
    for (int i = 0; i < rows; ++i) {
        result[i] = 0.0;
...
void OriginalLinearOperation::multiply_mv_col_major_opt(const double* matrix, int rows, int cols,
                                                  const double* vector, double* result) {
    #pragma omp parallel for
    for (int i = 0; i < rows; ++i) {
        result[i] = 0.0;
...
void OriginalLinearOperation::multiply_mm_naive_opt(const double* matrixA, int rowsA, int colsA,
                                               const double* matrixB, int rowsB, int colsB,
                                               double* result) {
    #pragma omp parallel for
    for (int i = 0; i < rowsA; ++i) {
...
void OriginalLinearOperation::multiply_mm_transposed_b_opt(const double* matrixA, int rowsA, int colsA,
                                                    const double* matrixB_transposed, int rowsB, int colsB, double* result) {
    #pragma omp parallel for
    for (int i = 0; i < rowsA; ++i) {
        const double* arow = matrixA + i * colsA;  // i th row start pointer of A
