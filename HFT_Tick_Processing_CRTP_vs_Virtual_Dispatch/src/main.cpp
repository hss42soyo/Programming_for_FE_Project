...
int main(int argc, char** argv) {
    ...
    if (argc > 1) n_ticks = static_cast(std::strtoul(argv[1], nullptr, 10));
    ...
    auto ns_free = run_bench("free_function", ticks,
        ...
        &{ return signal_free(q, a1, a2); }, iters);
    ...
    auto ns_crtp = run_bench("crtp_call", ticks,
        ...
        &{ return crtp.on_tick(q); }, iters);




    const double total_ops = static_cast(n_ticks) * iters;

    auto report = [&](const char*le ns_per_tick = ns / total_ops;
